...
function iconMarkup(name, url) { const label = escapeHtml(name || 'Unavailable'); return `<span class="icon-frame"><img class="item-icon" src="${imageSrc(url) || ''}" alt="" title="${label}" loading="lazy" onload="this.classList.add('is-loaded')" onerror="this.onerror=null;this.hidden=true;this.nextElementSibling.hidden=false"><span class="icon-fallback" aria-hidden="true" hidden>?</span></span>`; }
...
function revealApp() { document.documentElement.classList.remove('app-preload', 'preload-light'); }
function setSiteLoading(isLoading) { document.body.classList.toggle('is-loading', isLoading); siteSkeleton.hidden = !isLoading; revealApp(); }
...
function loadChampion(slug) { if (championRecords[slug]) return Promise.resolve(championRecords[slug]); return new Promise((resolve, reject) => { const script = document.createElement('script'); const timeout = window.setTimeout(() => reject(new Error('Record request timed out')), 12_000); const finish = (callback) => (value) => { window.clearTimeout(timeout); callback(value); }; script.src = `data/champions/${encodeURIComponent(slug)}.js`; script.dataset.champion = slug; script.onload = finish(() => championRecords[slug] ? resolve(championRecords[slug]) : reject(new Error('Record missing'))); script.onerror = finish(() => reject(new Error('Record request failed'))); document.head.appendChild(script); }); }
