...
The included GitHub Action fetches OP.GG for every champion every six hours and commits the refreshed `data/live.js` catalog back to the repository. You can also run it manually from the Actions tab, or run `npm run refresh` locally to refresh Gangplank only.
